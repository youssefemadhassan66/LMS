...
async function createMoreSessionReviews() {
  try {
    // Connect to database
    ...
    // Create session reviews for each student and session combination
    const reviewsToCreate = [];

    for (const student of students) {
      console.log(`\nProcessing student: ${student._id} (${student.userName || student.FullName})`);
    ...
    if (reviewsToCreate.length === 0) {
      console.log("\nNo new reviews to create. All sessions already have reviews for all students.");
      process.exit(0);
    }

    console.log(`\nCreating ${reviewsToCreate.length} new session reviews...`);
